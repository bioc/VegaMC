...
## (with the respective details) of each gene overlapping a significant region
    ...
    del_genes <- matrix(nrow=0, ncol=10)
    amp_genes <- matrix(nrow=0, ncol=10)
    if(baf==TRUE){
        loh_genes <- matrix(nrow=0, ncol=10)
    }

    message("Downloading Gene Informations from \"", ensembl_dataset,
        "\" Database")
    ...
    del_genes[,9] <- format(del_genes[,9], decimal.mark=",")
    ...
    amp_genes[,9] <- format(amp_genes[,9], decimal.mark=",")
    ...
    if(baf==TRUE){
        ...
        loh_genes[,9] <- format(loh_genes[,9], decimal.mark=",")
    }
